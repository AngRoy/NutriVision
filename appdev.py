...
# =============================
# MODEL (Renamed to match old checkpoint)
# =============================
class NutriVisionNetMultiHead(nn.Module):
    def __init__(self, food_dim=3, fv_dim=9, fast_dim=8, device="cuda", fine_tune_clip=False):
        ...
        # Heads must match old checkpoint naming:
        fuse_dim = 1024
        hidden_dim = 512

        # Old checkpoint uses "food_nutrition_head" not "food_head"
        self.food_nutrition_head = nn.Sequential(
            nn.Linear(fuse_dim, hidden_dim),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(hidden_dim, food_dim)  # e.g. 3 dims
        )
        ...
        # Old checkpoint uses "fastfood_head" not "fast_head"
        self.fastfood_head = nn.Sequential(
            nn.Linear(fuse_dim, hidden_dim),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(hidden_dim, fast_dim)  # e.g. 8 dims
        )
    ...
    def forward(self, img, source):
        ...
        return preds.squeeze(0).cpu().numpy().tolist(), caption
...
def run_inference(img, src, net, dev):
    with torch.no_grad():
        preds, capp = net(img, src)
    cals = preds[0] if preds else 0
    return preds, capp, cals
...
def show_register_form():
    ...
    if st.button("Register"):
        ...
        if reg_user == "" or reg_pw == "":
            ...
            success, msg, new_uid = register_user(
                reg_user, reg_pw,
...
with tabs[0]:
    ...
    if st.button("Refresh Dashboard"):
        # user can manually refresh or do nothing
        pass
...
with tabs[1]:
    ...
    upload_file = st.file_uploader("Choose an image (JPG, PNG)", type=["jpg", "jpeg", "png"])
    if upload_file:
        try:
